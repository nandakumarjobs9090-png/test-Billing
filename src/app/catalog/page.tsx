...
import { Plus, Search, Edit2, Trash2, Package, Inbox, Loader2, ImageIcon, Upload } from 'lucide-react';
...
export default function CatalogPage() {
  ...
  const handleSaveProduct = () => {
    ...
    const categoryToUse = isAddingNewCategory ? formData.newCategory : formData.category;
    
    if (!categoryToUse) {
      toast({ title: "Validation Error", description: "Please select or enter a category.", variant: "destructive" });
      return;
    }
    ...
    if (editingId) {
      const productDocRef = doc(db, 'users', user.uid, 'products', editingId);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        toast({ title: "Image Loaded", description: "Image ready to save." });
      };
      ...
      <main className="p-6 md:p-8">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground">Manage your product offerings and inventory.</p>
          </div>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-2">
                ...
                <div className="grid gap-3 border-t pt-4 mt-2">
                  ...
                  {formData.imageUrl && (
                    <div className="relative w-full h-40 rounded-lg overflow-hidden bg-muted border group">
                      <Image 
              ...
              <Card key={product.id} className="border-none shadow-sm bg-card hover:shadow-md transition-all overflow-hidden group">
                ...
                <CardContent className="p-4">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    <span className="text-[10px] bg-primary/5 text-primary px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">{product.category}</span>
                  </div>
                  <div className="flex items-center justify-between border-t border-dashed pt-4">
                    <div>
                      <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-tighter">Price</p>
                      <p className="font-bold text-accent">Rs.{product.price}</p>
                    </div>
                    <div className="text-right">
                      ...
                      <p className={cn(
                        ...
                        {product.stock} <span className="text-[10px] font-normal text-muted-foreground">u</span>
                      </p>
                    </div>
                  </div>
                </CardContent>
